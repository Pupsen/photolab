...
def reverse_video(path_to_open, path_to_save):
    ...
    for frame in frame_list+list(reversed(frame_list)):
        out.write(frame)
...
def reverse_audio(path_to_open, path_to_write):
    ...
    data = b''.join(full_data+full_data[::-1])
...
def run(file_dir, times):
    ...
    for i, time in enumerate(times):
        ...
        fragment = set_audio_to_video(reversed_video_path, reversed_audio_path)

        fragment.write_videofile('fragment-reverse-' + str(i) + '.mp4')

        fragments.append('fragment-reverse-' + str(i) + '.mp4')
